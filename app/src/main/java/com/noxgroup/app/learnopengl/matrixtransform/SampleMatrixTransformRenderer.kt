...
//            // Left face
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // Decode image and load it into texture
                GLES30.glActiveTexture(GLES30.GL_TEXTURE0)
                val bitmap = Utils.getImageFromAssetsFile("images/image2.png")
                GLES30.glBindTexture(GLES30.GL_TEXTURE_2D, imageTexture)
    ...
    override fun onParameterChange(parameterKey: String, parameterValue: Float) {
        when (parameterKey) {
            ...
            "lookAtEyeX" -> { cameraPositionX = parameterValue }
            "lookAtEyeY" -> { cameraPositionY = parameterValue }
            "lookAtEyeZ" -> { cameraPositionZ = parameterValue }
            "lookAtCenterX" -> { lookAtX = parameterValue }
            "lookAtCenterY" -> { lookAtY = parameterValue }
            "lookAtCenterZ" -> { lookAtZ = parameterValue }
            "lookAtUpX" -> { cameraUpX = parameterValue }
            "lookAtUpY" -> { cameraUpY = parameterValue }
            "lookAtUpZ" -> { cameraUpZ = parameterValue }
